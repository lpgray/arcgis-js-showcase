...
function initMap(options) {
  ...
  dojo.byId('mapInfoHeader').innerHTML = userConfig.i18n.viewer.sidePanel.map.label;
  ...
  dojo.byId('iosTooltip').innerHTML = userConfig.i18n.viewer.main.iosTooltip;
...
function initUI(map, response){
 // CONTAINER NUMBER
    ...
    // POPUP WINDOW EVENTS        
    dojo.connect(map.infoWindow, 'onShow', dojo.hitch(map, function () {
      selectOthers(map);
    }));
